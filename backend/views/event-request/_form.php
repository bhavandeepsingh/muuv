<?php

use yii\helpers\Html;
use yii\widgets\ActiveForm;

/* @var $this yii\web\View */
/* @var $model common\models\EventRequest */
/* @var $form yii\widgets\ActiveForm */
?>

<div class="event-request-form">

    <?php $form = ActiveForm::begin(); ?>

    <?= $form->field($model, 'event_requester_name')->textarea(['rows' => 6]) ?>

    <?= $form->field($model, 'event_requester_email')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'event_phone')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'event_name')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'event_comment')->textarea(['rows' => 6]) ?>

    <?= $form->field($model, 'event_url')->textInput(['maxlength' => true]) ?>

    <?= $form->field($model, 'event_type')->textInput() ?>

    <?= $form->field($model, 'event_id')->textInput() ?>

    <?= $form->field($model, 'created_at')->textInput() ?>

    <?= $form->field($model, 'updated_at')->textInput() ?>

    <div class="form-group">
        <?= Html::submitButton($model->isNewRecord ? Yii::t('app', 'Create') : Yii::t('app', 'Update'), ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary']) ?>
    </div>

    <?php ActiveForm::end(); ?>

</div>
