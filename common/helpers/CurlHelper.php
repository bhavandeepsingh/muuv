...
class CurlHelper {
    ...
    public function send(){
            ...
            $ch = curl_init();
            $str=$this->getEndPoint();
            if(empty($str)) throw new Exception("Please set end point for curl", 1);
            ...
            else if($this->curl_execute_type=='ARRAY'){
                    curl_setopt($ch, CURLOPT_POST, true);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($this->param_filter));
            }
            else{
